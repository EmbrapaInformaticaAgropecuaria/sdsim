...
public:
    ...
    void prja(const size_t neq, vector<double> &y, void (*f)(double, double*,double*),
              void *_data);

    void lsoda(void (*f)(double, double*,double*), const size_t neq, vector<double> &y,
               double *t, double tout, int itask, int *istate, int iopt, int jt,
               array<int, 7> &iworks, array<double, 4> &rworks, void *_data);

    void correction(const size_t neq, vector<double> &y, void (*f)(double, double*,double*),
                    size_t *corflag, double pnorm, double *del, double *delp,
                    double *told, size_t *ncf, double *rh, size_t *m,
                    void *_data);

    void stoda(const size_t neq, vector<double> &y, void (*f)(double, double*,double*),
               void *_data);

    // We call this function in VoxelPools::
    // void (*f)(double, double*,double*),
    void lsoda_update(void (*f)(double, double*,double*), const size_t neq,
