...
test_that("Coupled model connections", code = {
  ...
  mstatic <- expect_is(
    sdStaticModel(
      ...
      globalFunctions = list(b = function(x) { return(2*x)})),
    "sdStaticModel")
  bb <- expect_is(
    sdLoadModel(
      "BouncingBall", repository = TRUE), 
    "sdOdeModel")
  aren <- expect_is(
    sdLoadModel(
      "Arenstorf", repository = TRUE), 
    "sdOdeModel")
  aren$defaultScenario$input$testconEq <- 0
  aren$initialize(
    id = "arenstorf", 
    aux = c(aren$aux, testconEq = "inp$testconEq"))
  
  cm <- expect_is(
    sdCoupledModel(
      "testCons", 
      components = c(mstatic, bb, aren),
      connections = list(
  ...
  expect_true(all(outcm$auxTrajectory$arenstorf.testconEq == 
                    outcm$outTrajectory$test.eq2))
  
  expect_true(all(outcm$auxTrajectory$arenstorf.D1 == 
                    outcm$outTrajectory$test.D1con))
  
  expect_true(all(outcm$outTrajectory$BouncingBall.height == 
                    outcm$outTrajectory$test.heightcon))
})
...
test_that("Coupled model valid connections", code = {
  ...
  expect_true(
    coupledM$buildCoupledModel(
      from = 0, 
      to = 10, 
      by = 1), 
    info = "Test cases 1, 2 e 3")
  ...
  expect_warning(coupledM$addConnection(c("con5", "NA", "NA", "NA")), 
                 info = "Test case 5")
  ...
  expect_warning(coupledM$addConnection(c("con10", "SM", "y", "OM", "b")), 
                 info = "Test cases 10")
  
  expect_warning(coupledM$addConnection(c("con11", "SM", "y", "OM", "NA$b")), 
                 info = "Test cases 11")
})