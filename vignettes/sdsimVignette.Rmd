...
The model of the Lotka-Volterra equations presented in the deSolve package [@deSolve] will be implemented here using the package sdsim structure.
...
Where *dP* is the rate of change in the prey population and *dC* is the rate of change in the consumer population. The *growthP*, *ingestC* and *mortC* are auxiliary equations that help to compute the derivatives values and also store useful information. The auxiliary equations are defined as:
...
The sdsim atomic system dynamics model class named *sdModelClass* have the following fields that can be passed to the factory function:

- *modelId*: a string with a valid name for the model identification, uses the function *make.names* [@RCoreTeam] to make syntactically valid names.
- *modelDescription*: a string with a breath description of the model.
- *defaultScenario*: The model default scenario object. It should contain all the model variables initialized with default values that ensures the model simulation.
...
lvScenario <- sdScenario(scenarioId = "LVScenario",
                         state = state,
...
- *st*: is a list with the current estimate of the state variables in the ODE system;
...
- *aux*: is a list with the predecessors model auxiliary equations, following the sorted list, evaluated for the current time step.
...
These arguments are also available for the auxiliary equations expressions list, that can be defined in a list of strings or expressions which will be evaluated at each time step before the *differentialEquation* function call.
...
lv <- sdModel(modelId = "LotkaVolterra",
...
With the Lotka-Volterra model object initialized we can validate it's equations using the appropriate object method and then safely execute the simulation. This is done in the next chunk of code and the last ten lines of the outputted simulation trajectories is shown in a short print of the result object with the simulation diagnostics.
...
The output results could also be summarized and the trajectories fully printed for more accurate tabular visualization.

### sdsim Performance

Using the package *microbenchmark* [@microbenchmark] the performance of the Lotka-Volterra model implemented here was compared with the original implementation by the deSolve package. The simulation time elapse of the two implementations in a computer with 7.7 GiB of RAM memory and processor Intel® Core™ i7 CPU 870 @ 2.93GHz × 8 is compared in the following table.
...
A static model calculates the system in equilibrium, and thus is time-invariant. The sdsim static (or steady-state, no state variables) object model consists of algebraic equations and a default scenario describing the system environment (variables and values). In this definition a static model provides the system response to a specific set of input conditions. If there is no variant input, as a time series or results from a dynamic model, the evaluation of the algebraic equations will be constant.
...
envScen <- sdScenario(scenarioId = "EnvironmentScen",
                      parameter = parEnv,
                      input = inpEnv,
                      times = list(from = 0, to = 200, by = 1))

# create the static model of the environment
environmentLV <- sdStaticModel(staticModelId = "Environment",
...
The sdsim models have a default scenario that should contain all the model variables initialized with default values that ensure their simulation. The models composability capacity allow in simulations their combination with a range of scenarios, which overwrite the values of the model default scenario (see example in the following subsection [Using Excel Files]).
...
- input: The model inputs that are exogenous variables or time series variables.
...
The scenario fields are implemented as an active binding [@RCoreTeam], meaning they are associated with a function but are accessed as common items. This way the package enforces valid assignment to the object fields.
...
lvScenario$saveToXlsx(file = "LVscenario.xlsx")
```
...
When a specific scenario is passed to the simulator it is merged with the model default scenario, meaning that all the variables present in the given scenario will overwrite the default values in the simulation. 
...
The output plot can be configured with a character formula that indicates the variables trajectories to be used. The sdsim plot formula follow the standard:
...
coupledLV$validateODE(verbose = T)
...
preyScen <- sdScenario(scenarioId = "coupledPreyScen",
                       state = list(P = 5))

# creates a scenario for the consumer model
consumerScen <- sdScenario(scenarioId = "coupledConsumerScen",
                           state = list(C = 6))

# creates a scenario for the environment model
environmentScen <- sdScenario(scenarioId = "coupledEnvScen",
                              parameter = list(K = 15))

# creates the coupled scenario
coupledLvScen <- sdBuildCoupledScenario(coupledScenarioId = "LVCoupledScen",
                                        scenarios = list(Prey = preyScen,
