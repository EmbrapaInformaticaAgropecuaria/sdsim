...
The sdsim atomic system dynamics model class named *sdOdeModelClass* have the following fields that can be passed to the factory function:
...
- *aux*: A list with the model auxiliary equations in strings or R-expressions written in R-format to assist in the *ode* computation.
- *ode*: An R-function that computes the values of the state variables derivatives in the ODE system (the model definition) at each time step of the simulation.
...
lv <- sdOdeModel(id = "LotkaVolterra",
...
- state: The default initial state values for the ODE system. The state variables are used to describe the mathematical "state" of a dynamic system. The continuous rate of change of these variables is determined by the model *ode* function. Not present in static models.
...
consumer <- sdOdeModel(id = "Consumer",
...
1. An R-function that becomes zero when a root occur. When a root is found, the simulation triggers an event by calling the *EventFunction* or, if no *EventFunction* is defined, the simulation stops.
    ...
    - This function must be defined as $function(t, st, ct, par, inp, sw, aux)$, with the same arguments as the *ode* and it should return a numeric vector. If any element of this vector is zero an event is triggered.

The *EventFunction* must be defined as $function(t, st, ct, par, inp, sw, aux)$ with the same arguments as the *ode* and it should return the state-values (some of which modified), as a vector with the variables in the right order. If no *EventFunction* is defined, when a root is found the simulation stops.
